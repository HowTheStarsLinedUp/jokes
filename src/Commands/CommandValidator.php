...
class CommandValidator
{
    ...
    {
        return ($count > 250 or $count < 1) ?
            "Command invalid. 'count' must be above 1 and lower then 251." . PHP_EOL : false;
    }
    ...
    {
        return ($maxMarksPerJoke < 1) ?
            "Command invalid. 'maxMarksPerJoke' must be 1 or above." . PHP_EOL : false;
    }

    public function checkMaxMarkValue(int $maxMarkValue): bool|string
    {
        return ($maxMarkValue > 100 or $maxMarkValue < 1)
            ? "Command invalid. 'maxMarkValue' must be above 1 and lower then 101" . PHP_EOL : false;
    }

    public function checkPersonCount(int $personCount): bool|string
    {
        return ($personCount < 1) ?
            "Command invalid. 'personCount' must be 1 or above." . PHP_EOL : false;
    }
