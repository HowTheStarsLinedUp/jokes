...
class JsonWriterTest extends TestCase
{
    ...
    public function testJsonWriter()
    {
        ...
        $dotEnv->init();
        $fileName = $dotEnv->projectDir . DIRECTORY_SEPARATOR . 'tests' . DIRECTORY_SEPARATOR . 'jokesTest.json';

        try {
            $jokes = json_decode($this->jokesJson, true, flags: JSON_THROW_ON_ERROR);
            foreach($jokes as &$joke) {
                $joke = new Joke(...$joke);
            }
            unset($joke);

            (new JsonWriter())->write($jokes, $fileName);
        } catch (Exception $e) {
            echo $e->getMessage();
        }

        $this->assertFileExists($fileName);
        $this->assertEquals(time(), filemtime($fileName));
    }
}
