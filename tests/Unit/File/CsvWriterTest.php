...
class CsvWriterTest extends TestCase
{
    public function testCsvWriter()
    {
        ...
        $dotEnv->init();
        $fileName = $dotEnv->projectDir . DIRECTORY_SEPARATOR . 'tests' . DIRECTORY_SEPARATOR . 'jokesTest.csv';
        $jokesJson = <<<JOKESJSON
        [
          {
        ...
        try {
            $jokes = json_decode($jokesJson, true, flags: JSON_THROW_ON_ERROR);
            foreach($jokes as &$joke) {
                $joke = new Joke(...$joke);
            }
            unset($joke);

            (new CsvWriter())->write($jokes, $fileName);
        } catch (Exception $e) {
            echo $e->getMessage();
        }
